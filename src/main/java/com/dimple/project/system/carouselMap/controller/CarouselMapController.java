...
public class CarouselMapController extends BaseController {
    ...
    public AjaxResult editSave(@PathVariable String display, String carouselId) {
        return toAjax(carouselMapService.changeCarouselDisplay(carouselId,display,null));
    }
    ...
    @Log(title = "系统轮播图", businessType = BusinessType.UPDATE)
    @ResponseBody
    public AjaxResult editStatusSave(@PathVariable String status, String carouselId) {
    	return toAjax(carouselMapService.changeCarouselDisplay(carouselId,null,status));
    }
